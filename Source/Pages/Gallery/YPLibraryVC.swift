...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    //  选择模式
    public enum YPLibraryVCSelctedMode: Int {
        case all, video, image
    }
    
    public var selctedMode: YPLibraryVCSelctedMode = .all
    // MARK: - Init
    ...
    func initialize() {
        ...
        if YPConfig.library.onlyMultipleSelection {
        ...
        if let preselectedItems = YPConfig.library.preselectedItems, !preselectedItems.isEmpty {
            var mediaMode: YPLibraryVCSelctedMode = .all
