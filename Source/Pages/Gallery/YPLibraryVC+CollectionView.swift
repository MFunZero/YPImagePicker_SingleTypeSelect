...
extension YPLibraryVC {
    ...
    func checkLimit() {
        if YPConfig.library.singleMediaMultipleSelection {
            var hidden: Bool = true
            if let first = selection.first, first.index >= 0 {
                let lastAsset = mediaManager.fetchResult[first.index]
                if lastAsset.mediaType == .video {
                    if selection.count >= YPConfig.library.maxNumberOfVideo {
                        hidden = false
                        v.maxNumberWarningLabel.text = String(format: YPConfig.wordings.warningMaxVideoItemsLimit,
                        YPConfig.library.maxNumberOfVideo)
                    }
                } else {
                     if selection.count >= YPConfig.library.maxNumberOfImage {
                        hidden = false
                         v.maxNumberWarningLabel.text = String(format: YPConfig.wordings.warningMaxImageItemsLimit,
                         YPConfig.library.maxNumberOfImage)
                     }
                }
            }
            v.maxNumberWarningView.isHidden = hidden
        } else {
...
//        switch selctedMode {
//        case .image:
////            cell.isUserInteractionEnabled = asset.mediaType == .image
//            cell.multipleSelectionIndicator.isHidden = asset.mediaType != .image
//        case .video:
////            cell.isUserInteractionEnabled = asset.mediaType == .video
//            cell.multipleSelectionIndicator.isHidden = asset.mediaType != .video
//        default:
...
//        }
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if multipleSelectionEnabled {
            ...
            if YPConfig.library.singleMediaMultipleSelection {
                if let first = selection.first, first.index >= 0 {
                    let asset = mediaManager.fetchResult[first.index]
                    self.selctedMode = asset.mediaType == .image ? .image : .video
                } else {
                    if selection.count == 0 {
                        self.selctedMode = .all
                    }
                }
